        ...
        do {
            ...
            switch (menu) {
                ...
                case 2:
                    ...
                    System.out.println("1<< Transfer ke sesama Bank\n2<< Transfer ke Bank lain");
                    int goTo = sc.nextInt();
                    if (goTo == 1) {
                        ...
                        System.out.println("Nominal         :Rp."+cashTransfer);
                        ss.close();
                        socket.close();
                        dataInputStream.close();
                        ...
                        if(question==1){
                            ...
                            if(action.checkAuthorize(messages).equals("51")){
                                System.out.println("Saldo tidak cukup");
                            }else {
                                action.transfer(accountNumber, accountPIN, cashTransfer, accountNumberTransfer);
                            }
                    ...
                    if(nextStep==1){
                        ...
                        int newAmount = cashTransfer + 5500;
                        System.out.println("Potongan       : Rp. 5.500");
                        System.out.println("Total          :"+newAmount  );

                        System.out.println("Lanjutkan transaksi ?\n [1] Ya\n [2] Tidak ");
                        int question = sc.nextInt();
                        if (question == 1) {
                            //System.out.println(auth.getAccountBalance(accountNumber));
                            //System.out.println(action.getAccountBalance(action.transferSwitchingInquiry(accountNumber, accountPIN, cashTransfer, accountNumberTransfer)));
                            if(newAmount>auth.getAccountBalance(accountNumber)){
                                System.out.println("Saldo tidak cukup");
                            }else {
                ...
                case 3:
                    ...
                    if(produk==3){
                        ...
                        System.out.println("Nomor Rekening :"+token);
                        System.out.println("Nominal        :"+amount);
                        System.out.println("Lanjutkan transaksi ?\n 1 << Ya\n 2 << Tidak ");
                        int question = sc.nextInt();
                        if(question==1){
                            sendHttpRequest(action.purchaseInquiryPLN(accountNumber, accountPIN,amount));
                            ss = new ServerSocket(7777);
                            socket = ss.accept();
                            inputStream = socket.getInputStream();
                            dataInputStream = new DataInputStream(inputStream);
                            String messages = dataInputStream.readUTF();
                            //System.out.println(action.checkAuthorize(messages));
                            if(action.checkAuthorize(messages).equals("51")){
                                System.out.println("Saldo tidak cukup");
                            }else {
                                action.purchasePLN(accountNumber, accountPIN,amount);
                            }
                ...
                case 5:
                    ...
                    if(produk==4){
                        ...
                        System.out.println("Nomor Rekening :"+tokenNumber);
                        System.out.println("Nominal        :Rp.10000");
                    }
                        System.out.println("Lanjutkan transaksi ?\n 1 << Ya\n 2 << Tidak ");
                        int question = sc.nextInt();
                        if(question==1){
                            sendHttpRequest(action.paymentInquiryPAM(accountNumber, accountPIN));
                            ss = new ServerSocket(7777);
                            socket = ss.accept();
                            inputStream = socket.getInputStream();
                            dataInputStream = new DataInputStream(inputStream);
                            ...
                            if(action.checkAuthorize(messages).equals("51")){
                                ...
                                action.paymentPAM(accountNumber, accountPIN);
                            }
                            action.checkResponseCode(messages);
                            ss.close();
                            socket.close();
